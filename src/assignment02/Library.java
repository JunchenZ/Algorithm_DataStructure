...
public class Library {
  ...
  public ArrayList<LibraryBook> lookup(String holder) {
    ...
    for (LibraryBook libraryBook : library) {
      if (libraryBook.getHolder() == holder) {
        checkedOutBook.add(libraryBook);
      }
    }
  ...
  public boolean checkout(long isbn, String holder, int month, int day, int year) {
    for (LibraryBook libraryBook : library) {
      if (libraryBook.getIsbn() == isbn && libraryBook.isCheckedIn()) {
        libraryBook.setHolder(holder);
        libraryBook.setDueDate(new GregorianCalendar(year, month, day));
        return true;
      }
    }
    return false;
  ...
  public boolean checkin(String holder) {
    if (!lookup(holder).isEmpty()) {
      for (LibraryBook libraryBook : lookup(holder)) {
        checkin(libraryBook.getIsbn());
      }
      return true;
    }
    return false;
